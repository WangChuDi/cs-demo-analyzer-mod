...
func exportMatchToCSV(match *Match, outputPath string) error {
	...
	var writePlayers = func() {
		...
		for _, player := range match.Players() {
			line := []string{
				...
				converters.IntToString(player.FirstShotCount()),
				converters.IntToString(player.FirstShotHitCount()),
				converters.Float32ToString(player.FirstShotAccuracy()),
				converters.Float32ToString(player.CounterStrafingSuccessRate()),
				converters.Float64ToString(player.CounterStrafingAverageDeltaTick()),
				converters.Float64ToString(player.CounterStrafingDeltaStdDevTick()),
				converters.Float64ToString(player.CounterStrafingAToDAverageDeltaTick()),
				converters.Float32ToString(player.CounterStrafingAToDPerfectRate()),
				converters.Float64ToString(player.CounterStrafingDToAAverageDeltaTick()),
				converters.Float32ToString(player.CounterStrafingDToAPerfectRate()),
				converters.Float64ToString(player.CounterStrafingWToSAverageDeltaTick()),
				converters.Float32ToString(player.CounterStrafingWToSPerfectRate()),
				converters.Float64ToString(player.CounterStrafingSToWAverageDeltaTick()),
				converters.Float32ToString(player.CounterStrafingSToWPerfectRate()),
				converters.Float64ToString(player.CounterStrafingComboAverageDeltaTick()),
				converters.Float64ToString(player.CounterStrafingComboDeltaStdDevTick()),
				converters.Float32ToString(player.CounterStrafingComboPerfectRate()),
				converters.Float32ToString(player.CounterStrafingPerfectRate()),
				match.Checksum,
			}
	...
	var writeDataUtility = func() {
		...
		for _, utility := range match.Utilities {
			line := []string{
				...
				converters.BoolToString(utility.HasAttack),
				converters.BoolToString(utility.HasAttack2),
				converters.BoolToString(utility.HasJump),
				converters.BoolToString(utility.HasForward),
				converters.BoolToString(utility.HasBack),
				converters.BoolToString(utility.HasMoveLeft),
				converters.BoolToString(utility.HasMoveRight),
				converters.BoolToString(utility.HasWalk),
				converters.IntToString(utility.PinPulledTick),
				utility.GrenadeID,
				...
				converters.Float64ToString(utility.ThrowerSpeed2D),
				utility.ThrowerSpeedType,
				converters.Float32ToString(utility.ThrowerYaw),
				converters.Float32ToString(utility.ThrowerPitch),
				converters.BoolToString(utility.IsJumpThrow),
				converters.Float64ToString(utility.ThrowStrength),
				string(utility.MouseTypeByStrength),
				converters.Float64ToString(utility.InitialVelocityX),
				converters.Float64ToString(utility.InitialVelocityY),
				converters.Float64ToString(utility.InitialVelocityZ),
				converters.Float64ToString(utility.InitialSpeed),
				converters.Float64ToString(utility.InitialPositionX),
				converters.Float64ToString(utility.InitialPositionY),
				converters.Float64ToString(utility.InitialPositionZ),
				match.Checksum,
			}
			lines = append(lines, line)
		}
	...
	var writeAwpHoldDeaths = func() {
		header := []string{
			...
			"is victim slow",
			"is victim scoped",
			"is victim facing killer",
			"victim facing killer angle deg",
			"match checksum",
		}
